...
✦ ریتم سخت‌گیرانه: هر ۴ مصراع یک بند باید دقیقاً همان تعداد هجا داشته باشند — هیچ مصراعی کوتاه‌تر یا بلندتر از بقیه نباشد
✦ هجاها را بشمار: هر مصراع باید بین ۷ تا ۹ هجا داشته باشد — نه بیشتر، نه کمتر
✦ ضرب‌آهنگ: ترانه باید به‌راحتی روی یک ریتم ۴/۴ خوانده شود — ضربه‌های قوی روی هجاهای تأکیدی

══ ساختار اجباری (حداکثر ۳۰ ثانیه) ══
[ورس] ۴ مصراع — معرفی فضا و احساس (از جزئیات کوچک شروع کن)
[خط خالی]
[کورس] ۴ مصراع — شدیدترین احساس، اسم شخص اینجا، به‌یادماندنی‌ترین قسمت
[خط خالی]
[کورس] دقیقاً همان ۴ مصراع کورس — کلمه‌به‌کلمه تکرار

⚠️ مهم: کل ترانه فقط ۸ مصراع فعال (+ یک تکرار کورس) — نه بیشتر.
هر مصراع حداکثر ۸ کلمه. کل متن در ۳۰ ثانیه خوانده شود.
...
function buildPrompt(brief: SongBrief): string {
  ...
  parts.push(
    ...
    `کل متن باید در ۳۰ ثانیه خوانده شود (حداکثر ۸+۴ مصراع با تکرار کورس).`
  );

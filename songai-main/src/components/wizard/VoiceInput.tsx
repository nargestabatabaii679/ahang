        ...
        {recording ? (
          <>
            <div className="flex items-center gap-2 text-danger">
              {/* Kept intentionally: this pulse reflects a real, currently-true
                  state (the mic is live right now), the same convention every
                  camera/voice-memo app uses — not a decorative "for polish"
                  loop. Single instance, only visible while actually recording. */}
              <span className="h-3 w-3 animate-pulse rounded-full bg-danger" />
              <span className="nums text-lg font-bold">{fmt(seconds)}</span>
            </div>
            <button
              type="button"
              onClick={stopRecording}
              className="grid h-16 w-16 place-items-center rounded-full bg-danger text-white shadow-[0_0_0_8px_hsl(355_82%_64%/0.18)] transition"
              aria-label="پایان ضبط"
            >
              <Square className="h-6 w-6" weight="fill" />
            </button>
          ...
          <>
            <button
              ...
              className="grid h-16 w-16 place-items-center rounded-full bg-gradient-to-br from-tape to-thread text-primary-foreground shadow-[0_14px_40px_-12px_hsl(36_86%_55%/0.7)] transition hover:-translate-y-0.5"
              aria-label="شروع ضبط"
            >
              <Microphone className="h-7 w-7" />
            </button>
            <span className="text-sm font-bold">برای ضبط صدا بزن</span>
            <span className="text-xs text-muted-foreground">
